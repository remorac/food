 ...
 * Theme menu widget.
 */
class MenuHorizontal extends \yii\widgets\Menu
{
    /**
     * @inheritdoc
     */
    public $linkTemplate = '<a href="{url}" class="menu-link menu-toggle">{icon} {label}</a>';
    /**
     * @inheritdoc
     * Styles all labels of items on sidebar by AdminLTE
     */
    public $labelTemplate = '<span class="menu-text">{label}</span>';
    public $submenuTemplate = "\n<div class='menu-submenu'><ul class='menu-subnav' {show}>\n{items}\n</ul>\n";
    public $activateParents = true;
    public $defaultIconHtml = '<i class="menu-bullet menu-bullet-dot"><span></span></i> ';
    public $options = ['class' => 'menu-nav'];
    public $activeCssClass = 'menu-item-active menu-item-open';

    /**
     * @var string is prefix that will be added to $item['icon'] if it exist.
     * By default uses for Font Awesome (http://fontawesome.io/)
     */
    public static $iconClassPrefix = 'menu-icon ';
    ...
    protected function renderItem($item)
    {
        ...
        $replacements = [
            '{label}' => strtr($this->labelTemplate.(isset($item['items']) ? '<i class="menu-arrow"></i>' : ''), ['{label}' => $item['label'],]),
        ...
        foreach ($items as $i => $item) {
            ...
            if (!empty($item['items'])) {
				...
				if (isset($options['class'])) {
					$options['class'] .= ' menu-item menu-item-submenu';
				} else {
					$options['class'] = 'menu-item menu-item-submenu';
				}
            } else {
            ...
            if ($this->isItemActive($item)) $options['class'] .= ' menu-item-active';
